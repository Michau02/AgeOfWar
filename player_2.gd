extends Node
signal updateGoldP2
signal add_gold(player, reward)

@onready var HUD_progressBarAnimation = get_node("/root/Main/Camera2D/HUD/right_side/ProgressBar/ColorRect/AnimationPlayer")
@onready var HUD_progressBar = get_node("/root/Main/Camera2D/HUD/right_side/ProgressBar/ColorRect")
@onready var HUD_Queue_Label = get_node("/root/Main/Camera2D/HUD/right_side/Queue/Label")
@onready var HUD_Queue_NextEntity_icon = get_node("/root/Main/Camera2D/HUD/right_side/Queue/NextEntity_icon")
@onready var HUD_Gold = get_node("/root/Main/Camera2D/HUD/right_side/Gold_Exp_control/Gold_label")
...
func _ready():
	...
	base_scene.position = Vector2i(1900, 440)
	base_scene.player = 2
	add_child(base_scene)
	base_scene.show()

func _process(delta):
	if Input.is_action_just_pressed("unit1_P2"):
		spawn_unit(1)
	elif Input.is_action_just_pressed("unit2_P2"):
		spawn_unit(2)
	elif Input.is_action_just_pressed("unit3_P2"):
		spawn_unit(3)
	elif Input.is_action_just_pressed("buildTurret_P2"):
		build_turret()
	elif Input.is_action_just_pressed("sellTurret_P2"):
		sell_turret()
	elif Input.is_action_just_pressed("NextAge_P2"):
		next_age()
	if queue.size() == 0:
...
func build_turret():
	if turret == null and gold >= turret_cost[age-1]:
		emit_signal("add_gold", 2, -turret_cost[age-1], 0)
		...
		emit_signal("add_gold", 2, -(turret_cost[1] - turret_cost[0]), 0)
		turret.updateStats()

func sell_turret():
	if turret != null:
		gold += turret_sell[age-1]
		emit_signal("add_gold", 2, turret_sell[age-1], 0)
		turret.queue_free()
...
func _on_unit_1_timer_timeout():
	...
	new_unit.player = 2
	new_unit.position = Vector2(1820, 525)
	new_unit.age = age
	add_child(new_unit)
...
func _on_unit_3_timer_timeout():
	...
	new_unit.player = 2
	new_unit.position = Vector2(1820, 560)
	new_unit.age = age
	add_child(new_unit)
	new_unit.connect("unit_killed", _on_unit_killed)
	new_unit.show()
...
func _on_unit_killed(player, gold_reward, exp_reward):
	if player == 2:
		gold += gold_reward
		exp += exp_reward
		emit_signal("add_gold", 2, gold_reward, exp_reward)
